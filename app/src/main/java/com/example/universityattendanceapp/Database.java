...
public class  Database {
    ...
    private String url = "jdbc:mysql://localhost/3306/universityattendanceapp";
    private String user = "Adityas7'@'%'";
    private String pass = "Aditya@_073";
    ...
    public void addClass(Class c) {
        try {
            ...
            String create1 = "CREATE TABLE IF NOT EXISTS '" + c.getID()
                    + " - Sessio    ns' ('ID' integer, 'subject' text, 'Date' text);";
    ...
    public void deleteClass(int id) {
        try {
            String delete = "DELETE FROM `classes` where `id` = " + id + " - Sessions`;";
            statement.execute(delete);
            for (Session s : getSession(id)) {
                int sessionID = 0;
                deleteSession(id, s.getID(sessionID));
            }
    ...
    public void updateStudent(int classID, Student s) {
        try {
            String update = "UPDATE `" + classID + " - Student` SET `FirstName` = `" +
    ...
    public void deleteStudent(int classID, int studentID) {
        try {
            String delete = "DELETE FROM `" + classID + " - Student` WHERE `ID` = " + studentID + " ;";
            statement.execute(delete);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public Session getSession(int classID, int sessionID) {
        Session s = new Session();
        try {
            String select = "SELECT `TO`, `Subject`, `Date` FROM `" + classID +
    ...
    public void deleteSession(int classID, int sessionID) {
        ...
        public void removeStudentsFromSession ( int classID, int sessionID, ArrayList<
        Integer > students){
            try {
                for (Integer i : students) {
